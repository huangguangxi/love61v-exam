...
public class UserAction extends BaseAction {
	...
	public String beforeEditUser(Model model, String userId,String flag) {
		...
		if(StringUtils.isNotBlank(flag) && flag.equals("2")){//修改用户信息回选
			SystemUser user = systemUserService.getById(userId, SystemUser.class);
	...
	public Map<String, Object> editUser(Model model, SystemUser user) {
		...
		int count = 0;
		if(null != user && StringUtils.isNotBlank(user.getUserId())){//修改用户
	...
	@ResponseBody public Map<String, Object> deleteUser(String ids){
		Map<String, Object> map = getStatusMap();
		
		if(StringUtils.isNotBlank(ids)){
			String [] idArr = ids.split(",");
			int count = systemUserService.deleteBatch(Arrays.asList(idArr), SystemUser.class);
	...
	@ResponseBody public Map<String, Object> updatePwd(String newpwd,String oldpwd){
		Map<String, Object> map = getStatusMap();
